        ...
        const { selectProject } = await ui.run([
            {
                ...
                choices: projects.map((project) => {
                    return {
                        name: _rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green(project.packageName),
                        value: project
                    };
                }),
                pageSize: 12
...
/* harmony import */ var cli_table__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! cli-table */ 13593);
...
function greenUnderlineBold(text) {
    return _rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.underline(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.bold(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.green(text)));
}
function yellowUnderlineBold(text) {
    return _rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.underline(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.bold(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.yellow(text)));
}
function redUnderlineBold(text) {
    return _rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.underline(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.bold(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.red(text)));
}
function magentaUnderlineBold(text) {
    return _rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.underline(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.bold(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.magenta(text)));
}
const UI_GROUPS = [
    {
    ...
    {
        title: `${greenUnderlineBold('Missing.')} ${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.green('You probably want these.')}`,
        filter: { notInstalled: true, bump: undefined }
    },
    {
        title: `${greenUnderlineBold('Patch Update')} ${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.green('Backwards-compatible bug fixes.')}`,
        filter: { bump: 'patch' }
    },
    {
        title: `${yellowUnderlineBold('Minor Update')} ${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.yellow('New backwards-compatible features.')}`,
        bgColor: 'yellow',
        filter: { bump: 'minor' }
    },
    {
        title: `${redUnderlineBold('Major Update')} ${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.red('Potentially breaking API changes. Use caution.')}`,
        filter: { bump: 'major' }
    },
    {
        title: `${magentaUnderlineBold('Non-Semver')} ${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.magenta('Versions less than 1.0.0, caution.')}`,
...
function label(dep) {
    ...
    const name = _rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.yellow(dep.moduleName);
    const type = dep.devDependency ? _rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.green(' devDep') : '';
    const missing = dep.notInstalled ? _rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.red(' missing') : '';
    const homepage = dep.homepage ? _rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.blue(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.underline(dep.homepage)) : '';
    return [
        name + type + missing,
        installed,
        installed && '>',
        _rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.Colorize.bold(dep.latest || ''),
...
function unselectable(options) {
    return new (inquirer__WEBPACK_IMPORTED_MODULE_0___default().Separator)(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_2__.AnsiEscape.removeCodes(options ? options.title : ''));
}
function createChoices(packages, options) {
    ...
    const choices = filteredChoices
        .map(getChoice)
        .filter(Boolean);
    const cliTable = new (cli_table__WEBPACK_IMPORTED_MODULE_1___default())({
        chars: {
            top: '',
            'top-mid': '',
            'top-left': '',
...
class SearchListPrompt extends (inquirer_lib_prompts_base__WEBPACK_IMPORTED_MODULE_1___default()) {
    ...
    render(error) {
        ...
        if (this._firstRender) {
            message += _rushstack_terminal__WEBPACK_IMPORTED_MODULE_0__.Colorize.dim(' (Use arrow keys)');
        }
        // Render choices or answer depending on the state
        if (this.status === 'answered') {
            message += _rushstack_terminal__WEBPACK_IMPORTED_MODULE_0__.Colorize.cyan(this.opt.choices.getChoice(this._selected).short);
        }
        else {
            ...
            message += `\n${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_0__.Colorize.white(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_0__.Colorize.bold('Start typing to filter:'))} ${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_0__.Colorize.cyan(this._query)}`;
            // @ts-expect-error Types are wrong
            message += '\n' + this._paginator.paginate(choicesStr, realIndexPosition, this.opt.pageSize);
        }
        if (error) {
            bottomContent = _rushstack_terminal__WEBPACK_IMPORTED_MODULE_0__.Colorize.red('>> ') + error;
        }
...
function listRender(choices, pointer) {
    ...
    choices.forEach((choice, i) => {
        ...
        if (!choice.disabled) {
            ...
            if (i === pointer) {
                output += _rushstack_terminal__WEBPACK_IMPORTED_MODULE_0__.Colorize.cyan((figures__WEBPACK_IMPORTED_MODULE_4___default().pointer) + line);
            }
            else {
                output += ' ' + line;
            }
