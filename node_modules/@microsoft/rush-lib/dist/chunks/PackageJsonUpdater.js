...
/* harmony import */ var _InstallManagerFactory__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./InstallManagerFactory */ 611779);
/* harmony import */ var _versionMismatch_VersionMismatchFinder__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./versionMismatch/VersionMismatchFinder */ 936800);
/* harmony import */ var _PurgeManager__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./PurgeManager */ 371186);
/* harmony import */ var _utilities_Utilities__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ../utilities/Utilities */ 827501);
/* harmony import */ var _api_PackageJsonEditor__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../api/PackageJsonEditor */ 73666);
/* harmony import */ var _versionMismatch_VersionMismatchFinderProject__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./versionMismatch/VersionMismatchFinderProject */ 686807);
/* harmony import */ var _RushConstants__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./RushConstants */ 951904);
/* harmony import */ var _installManager_InstallHelpers__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./installManager/InstallHelpers */ 741340);
/* harmony import */ var _PackageJsonUpdaterTypes__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ./PackageJsonUpdaterTypes */ 427915);
...
class PackageJsonUpdater {
    constructor(rushConfiguration, rushGlobalFolder) {
        ...
        this._terminalProvider = new _rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.ConsoleTerminalProvider();
        this._terminal = new _rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Terminal(this._terminalProvider);
    }
    /**
    ...
    async doRushUpgradeAsync(options) {
        ...
        for (const { moduleName, latest: latestVersion, packageJson, devDependency, peerDependency } of packagesToAdd) {
            ...
            this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green(`Updating projects to use `) + moduleName + '@' + _rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.cyan(version));
        ...
        for (const project of projects) {
            const mismatchFinderProject = new _versionMismatch_VersionMismatchFinderProject__WEBPACK_IMPORTED_MODULE_2__.VersionMismatchFinderProject(project);
            const currentProjectDepUpdate = {
                project: mismatchFinderProject,
                dependenciesToAddOrUpdateOrRemove: dependenciesToUpdate,
                dependencyType: _api_PackageJsonEditor__WEBPACK_IMPORTED_MODULE_3__.DependencyType.Regular
            };
            const currentProjectDevDepUpdate = {
                project: mismatchFinderProject,
                dependenciesToAddOrUpdateOrRemove: devDependenciesToUpdate,
                dependencyType: _api_PackageJsonEditor__WEBPACK_IMPORTED_MODULE_3__.DependencyType.Dev
            };
        ...
        if (updateOtherPackages) {
            const mismatchFinder = _versionMismatch_VersionMismatchFinder__WEBPACK_IMPORTED_MODULE_4__.VersionMismatchFinder.getMismatches(this._rushConfiguration, options);
        ...
        for (const [filePath, project] of allPackageUpdates) {
            if (project.saveIfModified()) {
                this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green('Wrote ') + filePath);
            }
        }
        if (!skipUpdate) {
    ...
    async doRushUpdateAsync(options) {
        ...
        for (const { project } of allPackageUpdates) {
            if (project.saveIfModified()) {
                this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green('Wrote'), project.filePath);
            }
        }
        if (!skipUpdate) {
            if (this._rushConfiguration.subspacesFeatureEnabled) {
    ...
    async _doUpdateAsync(debugInstall, subspace, variant) {
        this._terminal.writeLine();
        this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green('Running "rush update"'));
        this._terminal.writeLine();
        const purgeManager = new _PurgeManager__WEBPACK_IMPORTED_MODULE_5__.PurgeManager(this._rushConfiguration, this._rushGlobalFolder);
        const installManagerOptions = {
            ...
            variant,
            maxInstallAttempts: _RushConstants__WEBPACK_IMPORTED_MODULE_6__.RushConstants.defaultMaxInstallAttempts,
        ...
        const installManager = await _InstallManagerFactory__WEBPACK_IMPORTED_MODULE_7__.InstallManagerFactory.getInstallManagerAsync(this._rushConfiguration, this._rushGlobalFolder, purgeManager, installManagerOptions);
        try {
    ...
    async _updateProjectsAsync(subspace, dependencyAnalyzer, options) {
        ...
        for (const { packageName, version: initialVersion, rangeStyle } of packagesToUpdate) {
            ...
            this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green('Updating projects to use '), `${packageName}@`, _rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.cyan(version));
            this._terminal.writeLine();
        ...
        for (const project of subspaceProjects) {
            const currentProjectUpdate = {
                project: new _versionMismatch_VersionMismatchFinderProject__WEBPACK_IMPORTED_MODULE_2__.VersionMismatchFinderProject(project),
                dependenciesToAddOrUpdateOrRemove: dependenciesToAddOrUpdate,
                dependencyType: devDependency ? _api_PackageJsonEditor__WEBPACK_IMPORTED_MODULE_3__.DependencyType.Dev : peerDependency ? _api_PackageJsonEditor__WEBPACK_IMPORTED_MODULE_3__.DependencyType.Peer : undefined
            };
            this.updateProject(currentProjectUpdate);
            let otherPackageUpdates = [];
            // we need to do a mismatch check
            if (updateOtherPackages) {
                const mismatchFinder = _versionMismatch_VersionMismatchFinder__WEBPACK_IMPORTED_MODULE_4__.VersionMismatchFinder.getMismatches(this._rushConfiguration, {
                    subspace,
                    variant
                });
        ...
        for (const [packageName, newVersion] of Object.entries(dependenciesToAddOrUpdateOrRemove)) {
            ...
            const oldDependencyType = oldDevDependency
                ...
                : oldDependency
                    ...
                    : undefined;
            dependencyType = dependencyType || oldDependencyType || _api_PackageJsonEditor__WEBPACK_IMPORTED_MODULE_3__.DependencyType.Regular;
    ...
    async _getNormalizedVersionSpecAsync(projects, packageName, initialSpec, implicitlyPreferredVersion, explicitlyPreferredVersion, rangeStyle, ensureConsistentVersions) {
        this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.gray(`Determining new version for dependency: ${packageName}`));
        if (initialSpec) {
            this._terminal.writeLine(`Specified version selector: ${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.cyan(initialSpec)}`);
        }
        else {
        ...
        if (initialSpec) {
            if (initialSpec === implicitlyPreferredVersion) {
                this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green('Assigning "') +
                    _rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.cyan(initialSpec) +
                    _rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green(`" for "${packageName}" because it matches what other projects are using in this repo.`));
                return initialSpec;
            }
            if (initialSpec === explicitlyPreferredVersion) {
                this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green('Assigning "') +
                    _rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.cyan(initialSpec) +
                    _rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.green(`" for "${packageName}" because it is the preferred version listed in ${_RushConstants__WEBPACK_IMPORTED_MODULE_6__.RushConstants.commonVersionsFilename}.`));
                return initialSpec;
            }
        }
        if (ensureConsistentVersions && !initialSpec) {
            if (implicitlyPreferredVersion) {
                this._terminal.writeLine(`Assigning the version "${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.cyan(implicitlyPreferredVersion)}" for "${packageName}" ` +
                    'because it is already used by other projects in this repo.');
                return implicitlyPreferredVersion;
            }
            if (explicitlyPreferredVersion) {
                this._terminal.writeLine(`Assigning the version "${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.cyan(explicitlyPreferredVersion)}" for "${packageName}" ` +
                    `because it is the preferred version listed in ${_RushConstants__WEBPACK_IMPORTED_MODULE_6__.RushConstants.commonVersionsFilename}.`);
                return explicitlyPreferredVersion;
            }
        }
        await _installManager_InstallHelpers__WEBPACK_IMPORTED_MODULE_8__.InstallHelpers.ensureLocalPackageManagerAsync(this._rushConfiguration, this._rushGlobalFolder, _RushConstants__WEBPACK_IMPORTED_MODULE_6__.RushConstants.defaultMaxInstallAttempts);
        ...
        if (initialSpec && initialSpec !== 'latest') {
            this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.gray('Finding versions that satisfy the selector: ') + initialSpec);
            this._terminal.writeLine();
            if (localProject !== undefined) {
                const version = localProject.packageJson.version;
                if (semver__WEBPACK_IMPORTED_MODULE_0__.satisfies(version, initialSpec)) {
                ...
                const allVersions = await _utilities_Utilities__WEBPACK_IMPORTED_MODULE_9__.Utilities.executeCommandAndCaptureOutputAsync(this._rushConfiguration.packageManagerToolFilename, commandArgs, this._rushConfiguration.commonTempFolder);
                let versionList;
                if (this._rushConfiguration.packageManager === 'yarn') {
                ...
                this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.gray(`Found ${versionList.length} available versions.`));
                for (const version of versionList) {
                    if (semver__WEBPACK_IMPORTED_MODULE_0__.satisfies(version, initialSpec)) {
                        selectedVersion = initialSpec;
                        this._terminal.writeLine(`Found a version that satisfies ${initialSpec}: ${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.cyan(version)}`);
                        break;
                    }
                }
                if (!selectedVersion) {
            ...
            if (localProject !== undefined) {
                ...
                if (!this._rushConfiguration.ensureConsistentVersions) {
                    this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.gray(`The "ensureConsistentVersions" policy is NOT active, so we will assign the latest version.`));
                    this._terminal.writeLine();
                }
                ...
                selectedVersion = (await _utilities_Utilities__WEBPACK_IMPORTED_MODULE_9__.Utilities.executeCommandAndCaptureOutputAsync(this._rushConfiguration.packageManagerToolFilename, commandArgs, this._rushConfiguration.commonTempFolder)).trim();
            }
            this._terminal.writeLine();
            this._terminal.writeLine(`Found latest version: ${_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.cyan(selectedVersion)}`);
        }
        this._terminal.writeLine();
        let reasonForModification = '';
        if (selectedVersion !== '*') {
            switch (rangeStyle) {
                case _PackageJsonUpdaterTypes__WEBPACK_IMPORTED_MODULE_10__.SemVerStyle.Caret: {
                    selectedVersionPrefix += '^';
                    reasonForModification = ' because the "--caret" flag was specified';
                    break;
                }
                case _PackageJsonUpdaterTypes__WEBPACK_IMPORTED_MODULE_10__.SemVerStyle.Exact: {
                    reasonForModification = ' because the "--exact" flag was specified';
                    break;
                }
                case _PackageJsonUpdaterTypes__WEBPACK_IMPORTED_MODULE_10__.SemVerStyle.Tilde: {
                    selectedVersionPrefix += '~';
                    break;
                }
                case _PackageJsonUpdaterTypes__WEBPACK_IMPORTED_MODULE_10__.SemVerStyle.Passthrough: {
                    break;
                }
                default: {
        ...
        this._terminal.writeLine(_rushstack_terminal__WEBPACK_IMPORTED_MODULE_1__.Colorize.gray(`Assigning version "${normalizedVersion}" for "${packageName}"${reasonForModification}.`));
    ...
    _tryGetLocalProject(packageName, projects) {
        ...
        if (project === foundProject) {
            throw new Error('Unable to add a project as a dependency of itself unless the dependency is listed as a cyclic dependency ' +
                `in ${_RushConstants__WEBPACK_IMPORTED_MODULE_6__.RushConstants.rushJsonFilename}. This command attempted to add "${foundProject.packageName}" ` +
    ...
    _cheaplyDetectSemVerRangeStyle(version) {
        ...
        switch (version[0]) {
            case '~':
                return _PackageJsonUpdaterTypes__WEBPACK_IMPORTED_MODULE_10__.SemVerStyle.Tilde;
            case '^':
                return _PackageJsonUpdaterTypes__WEBPACK_IMPORTED_MODULE_10__.SemVerStyle.Caret;
            default:
                this._terminal.writeLine(`No SemVer range detected for version: ${version}. The exact version will be set in package.json.`);
                return _PackageJsonUpdaterTypes__WEBPACK_IMPORTED_MODULE_10__.SemVerStyle.Exact;
        }
    }
