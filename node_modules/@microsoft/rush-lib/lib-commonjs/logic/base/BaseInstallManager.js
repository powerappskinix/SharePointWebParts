...
exports.BaseInstallManager = exports.pnpmIgnoreCompatibilityDbParameter = void 0;
...
const PolicyValidator = __importStar(require("../policy/PolicyValidator"));
...
const SubspacePnpmfileConfiguration_1 = require("../pnpm/SubspacePnpmfileConfiguration");
const ProjectImpactGraphGenerator_1 = require("../ProjectImpactGraphGenerator");
 ...
 */
exports.pnpmIgnoreCompatibilityDbParameter = '--config.ignoreCompatibilityDb';
...
class BaseInstallManager {
    constructor(rushConfiguration, rushGlobalFolder, purgeManager, options) {
        ...
        this._commonTempLinkFlag = new FlagFile_1.FlagFile(options.subspace.getSubspaceTempFolderPath(), RushConstants_1.RushConstants.lastLinkFlagFilename, {});
        this.subspaceInstallFlags = new Map();
        if (rushConfiguration.subspacesFeatureEnabled) {
            for (const subspace of rushConfiguration.subspaces) {
                this.subspaceInstallFlags.set(subspace.subspaceName, (0, LastInstallFlag_1.getCommonTempFlag)(rushConfiguration, subspace));
            }
        }
    }
    async doInstallAsync() {
        ...
        if (isFilteredInstall && !useWorkspaces) {
            ...
            console.log();
            // eslint-disable-next-line no-console
            console.log(terminal_1.Colorize.red('Project filtering arguments can only be used when running in a workspace environment. Run the ' +
        ...
        if (allowShrinkwrapUpdates && isFilteredInstall) {
            ...
            if (!this.rushConfiguration.subspacesFeatureEnabled) {
                ...
                console.log();
                // eslint-disable-next-line no-console
                console.log(terminal_1.Colorize.red('Project filtering arguments cannot be used when running "rush update". Run the command again ' +
        ...
        const projectImpactGraphGenerator = this.rushConfiguration
            ...
            ? new ProjectImpactGraphGenerator_1.ProjectImpactGraphGenerator(this._terminal, this.rushConfiguration)
            : undefined;
        ...
        // eslint-disable-next-line no-console
        console.log('\n' + terminal_1.Colorize.bold(`Checking installation in "${subspace.getSubspaceTempFolderPath()}"`));
        ...
        const commonTempInstallFlag = (0, LastInstallFlag_1.getCommonTempFlag)(this.rushConfiguration, subspace, {
        ...
        const canSkipInstallAsync = async () => {
            ...
            const outputStats = await node_core_library_1.FileSystem.getStatisticsAsync(commonTempInstallFlag.path);
        ...
        if (resolutionOnly ||
            ...
            if (!this.rushConfiguration.rushConfigurationJson.suppressRushIsPublicVersionCheck) {
                ...
                if (publishedRelease === false) {
                    // eslint-disable-next-line no-console
                    console.log(terminal_1.Colorize.yellow('Warning: This release of the Rush tool was unpublished; it may be unstable.'));
                }
            }
            if (!resolutionOnly) {
            ...
            if (this.options.allowShrinkwrapUpdates && !shrinkwrapIsUpToDate) {
                ...
                const shrinkwrapFile = ShrinkwrapFileFactory_1.ShrinkwrapFileFactory.getShrinkwrapFile(this.rushConfiguration.packageManager, this.rushConfiguration.packageManagerOptions, committedShrinkwrapFileName);
                ...
                Utilities_1.Utilities.syncFile(subspace.getTempShrinkwrapFilename(), committedShrinkwrapFileName);
            }
            else {
            ...
            if (this.options.allowShrinkwrapUpdates) {
                if (subspace.getRepoState().refreshState(this.rushConfiguration, subspace, variant)) {
                    // eslint-disable-next-line no-console
                    console.log(terminal_1.Colorize.yellow(`${RushConstants_1.RushConstants.repoStateFilename} has been modified and must be committed to source control.`));
                }
            }
        }
        else {
        ...
        if (this.rushConfiguration.packageManager === 'pnpm' && (experiments === null || experiments === void 0 ? void 0 : experiments.usePnpmSyncForInjectedDependencies)) {
            ...
            if ((await node_core_library_1.FileSystem.existsAsync(pnpmLockfilePath)) && (await node_core_library_1.FileSystem.existsAsync(dotPnpmFolder))) {
                await (0, pnpm_sync_lib_1.pnpmSyncPrepareAsync)({
                    ...
                    readPnpmLockfile: async (lockfilePath) => {
                        var _a, _b;
                        const wantedPnpmLockfile = PnpmShrinkwrapFile_1.PnpmShrinkwrapFile.loadFromFile(lockfilePath, { withCaching: true });
            ...
            for (const rushProject of subspace.getProjects()) {
                ...
                if (!(0, fs_1.existsSync)(pnpmSyncJsonPath)) {
                    continue;
                }
                let existingPnpmSyncJsonFile;
                try {
                    existingPnpmSyncJsonFile = JSON.parse((await (0, promises_1.readFile)(pnpmSyncJsonPath)).toString());
                    if ((existingPnpmSyncJsonFile === null || existingPnpmSyncJsonFile === void 0 ? void 0 : existingPnpmSyncJsonFile.version) !== (0, pnpm_sync_lib_1.pnpmSyncGetJsonVersion)()) {
                        await (0, promises_1.unlink)(pnpmSyncJsonPath);
                    }
                }
                catch (e) {
                    await (0, promises_1.unlink)(pnpmSyncJsonPath);
    ...
    async canSkipInstallAsync(lastModifiedDate, subspace, variant) {
        ...
        potentiallyChangedFiles.push(path.join(subspace.getSubspaceTempFolderPath(), RushConstants_1.RushConstants.nodeModulesFolderName));
        ...
        if (this.rushConfiguration.packageManager === 'pnpm') {
            ...
            const pnpmFileFilePath = subspace.getPnpmfilePath();
            const pnpmFileExists = await node_core_library_1.FileSystem.existsAsync(pnpmFileFilePath);
            if (pnpmFileExists) {
                potentiallyChangedFiles.push(pnpmFileFilePath);
            }
        }
        return await Utilities_1.Utilities.isFileTimestampCurrentAsync(lastModifiedDate, potentiallyChangedFiles);
    }
    async prepareAsync(subspace, variant, projectImpactGraphGenerator) {
        ...
        const approvedPackagesChecker = new ApprovedPackagesChecker_1.ApprovedPackagesChecker(this.rushConfiguration);
        if (approvedPackagesChecker.approvedPackagesFilesAreOutOfDate) {
            ...
            if (allowShrinkwrapUpdates) {
                terminal.writeLine(terminal_1.Colorize.yellow('Approved package files have been updated. These updates should be committed to source control'));
            }
            else {
        ...
        await InstallHelpers_1.InstallHelpers.ensureLocalPackageManagerAsync(this.rushConfiguration, this.rushGlobalFolder, this.options.maxInstallAttempts);
        ...
        if (!this.options.fullUpgrade) {
            const committedShrinkwrapFileName = subspace.getCommittedShrinkwrapFilePath(variant);
            try {
                shrinkwrapFile = ShrinkwrapFileFactory_1.ShrinkwrapFileFactory.getShrinkwrapFile(this.rushConfiguration.packageManager, this.rushConfiguration.packageManagerOptions, committedShrinkwrapFileName);
            }
            catch (ex) {
                terminal.writeLine();
                ...
                if (!allowShrinkwrapUpdates) {
                    terminal.writeLine();
                    terminal.writeLine(terminal_1.Colorize.red('You need to run "rush update" to fix this problem'));
                    throw new node_core_library_1.AlreadyReportedError();
                }
        ...
        const variantIsUpToDate = !(await node_core_library_1.JsonFile.saveAsync(currentVariantJson, currentVariantJsonFilePath, {
            onlyIfChanged: true
        }));
        this.rushConfiguration._currentVariantJsonLoadingPromise = undefined;
        if (this.options.variant) {
            terminal.writeLine();
            terminal.writeLine(terminal_1.Colorize.bold(`Using variant '${this.options.variant}' for installation.`));
        }
        else if (!variantIsUpToDate && !variant && this.rushConfiguration.variants.size > 0) {
            terminal.writeLine();
            terminal.writeLine(terminal_1.Colorize.bold('Using the default variant for installation.'));
        }
        ...
        if (this.rushConfiguration.subspacesFeatureEnabled) {
            ...
            try {
                commonNpmrcFileLines = (await node_core_library_1.FileSystem.readFileAsync(commonNpmrcPath)).split('\n');
            }
            catch (e) {
            ...
            extraNpmrcLines.push(`global-pnpmfile=${subspace.getSubspaceTempFolderPath()}/${RushConstants_1.RushConstants.pnpmfileGlobalFilename}`);
        }
        ...
        const npmrcText = Utilities_1.Utilities.syncNpmrc({
        ...
        if (this.rushConfiguration.packageManager === 'pnpm') {
            ...
            const commonTempPnpmPatchesFolder = `${subspace.getSubspaceTempFolderPath()}/${RushConstants_1.RushConstants.pnpmPatchesFolderName}`;
            const rushPnpmPatchesFolder = subspace.getSubspacePnpmPatchesFolderPath();
            let rushPnpmPatches;
            try {
                rushPnpmPatches = await node_core_library_1.FileSystem.readFolderItemsAsync(rushPnpmPatchesFolder);
            }
            catch (e) {
                if (!node_core_library_1.FileSystem.isNotExistError(e)) {
                    throw e;
            ...
            if (rushPnpmPatches) {
                await node_core_library_1.FileSystem.ensureFolderAsync(commonTempPnpmPatchesFolder);
                const existingPatches = await node_core_library_1.FileSystem.readFolderItemsAsync(commonTempPnpmPatchesFolder);
                const copiedPatchNames = new Set();
                await node_core_library_1.Async.forEachAsync(rushPnpmPatches, async (patch) => {
                ...
                await node_core_library_1.Async.forEachAsync(existingPatches, async (patch) => {
                    const name = patch.name;
                    if (!copiedPatchNames.has(name)) {
                        await node_core_library_1.FileSystem.deleteFileAsync(`${commonTempPnpmPatchesFolder}/${name}`);
                    }
                }, { concurrency: 50 });
            }
            else {
                await node_core_library_1.FileSystem.deleteFolderAsync(commonTempPnpmPatchesFolder);
            }
        }
        // Shim support for pnpmfile in.
        // Additionally when in workspaces, the shim implements support for common versions.
        if (this.rushConfiguration.packageManager === 'pnpm') {
            await PnpmfileConfiguration_1.PnpmfileConfiguration.writeCommonTempPnpmfileShimAsync(this.rushConfiguration, subspace.getSubspaceTempFolderPath(), subspace, variant);
            if (this.rushConfiguration.subspacesFeatureEnabled) {
                await SubspacePnpmfileConfiguration_1.SubspacePnpmfileConfiguration.writeCommonTempSubspaceGlobalPnpmfileAsync(this.rushConfiguration, subspace, variant);
            }
        }
        ...
        if (shrinkwrapWarnings.length > 0) {
            terminal.writeLine();
            terminal.writeLine(terminal_1.Colorize.yellow(terminal_1.PrintUtilities.wrapWords(`The ${this.rushConfiguration.shrinkwrapFilePhrase} contains the following issues:`)));
            for (const shrinkwrapWarning of shrinkwrapWarnings) {
                terminal.writeLine(terminal_1.Colorize.yellow('  ' + shrinkwrapWarning));
            }
            terminal.writeLine();
        }
        let hasErrors = false;
        ...
        if (!projectImpactGraphIsUpToDate && !allowShrinkwrapUpdates) {
            hasErrors = true;
            terminal.writeErrorLine();
            terminal.writeErrorLine(terminal_1.Colorize.red(`The ${RushConstants_1.RushConstants.projectImpactGraphFilename} file is missing or out of date. You need to run "rush update".`));
        }
        if (hasErrors) {
            throw new node_core_library_1.AlreadyReportedError();
        }
    ...
    async _installGitHooksAsync() {
        ...
        if (node_core_library_1.FileSystem.exists(hookSource) && hookDestination) {
            ...
            if (hookFilenames.length > 0) {
                // eslint-disable-next-line no-console
                console.log('\n' + terminal_1.Colorize.bold('Found files in the "common/git-hooks" folder.'));
                if (!(await git.getIsHooksPathDefaultAsync())) {
                    const hooksPath = await git.getConfigHooksPathAsync();
                    const color = this.options.bypassPolicy ? terminal_1.Colorize.yellow : terminal_1.Colorize.red;
                    // eslint-disable-next-line no-console
                    console.error(color([
                        ' ',
                    ...
                    console.error(color([
                        '(Or, to temporarily ignore this problem, invoke Rush with the ' +
                            `"${RushConstants_1.RushConstants.bypassPolicyFlagLongName}" option.)`,
                        ' '
                    ].join('\n')));
                    throw new node_core_library_1.AlreadyReportedError();
                }
                ...
                const hookRelativePath = node_core_library_1.Path.convertToSlashes(path.relative(hookDestination, hookSource));
                ...
                for (const filename of filteredHookFilenames) {
                    ...
                    const originalHookFileContent = node_core_library_1.FileSystem.readFile(hookFilePath);
                    node_core_library_1.FileSystem.writeFile(hookFilePath, originalHookFileContent, {
                    ...
                    const originalPosixModeBits = node_core_library_1.FileSystem.getPosixModeBits(hookFilePath);
                    node_core_library_1.FileSystem.changePosixModeBits(hookFilePath, 
                    // eslint-disable-next-line no-bitwise
                    originalPosixModeBits | node_core_library_1.PosixModeBits.UserRead | node_core_library_1.PosixModeBits.UserExecute);
...
`;
    ...
    pushConfigurationArgs(args, options, subspace) {
        ...
        if (this.rushConfiguration.packageManager === 'npm') {
            ...
            const { pnpmVerifyStoreIntegrity } = EnvironmentConfiguration_1.EnvironmentConfiguration;
            ...
            */
            const isAutoInstallPeersInNpmrc = (0, npmrcUtilities_1.isVariableSetInNpmrcFile)(subspace.getSubspaceConfigFolderPath(), 'auto-install-peers');
            ...
            */
            const isResolutionModeInNpmrc = (0, npmrcUtilities_1.isVariableSetInNpmrcFile)(subspace.getSubspaceConfigFolderPath(), 'resolution-mode');
            ...
            if (semver.satisfies(this.rushConfiguration.packageManagerToolVersion, '6.32.12 - 6.33.x || 7.0.1 - 7.8.x')) {
                this._terminal.writeWarningLine(`Warning: Your ${RushConstants_1.RushConstants.rushJsonFilename} specifies a pnpmVersion with a known issue ` +
            ...
            if (semver.gte(this.rushConfiguration.packageManagerToolVersion, '7.9.0') ||
                ...
                args.push(exports.pnpmIgnoreCompatibilityDbParameter);
            }
        }
    ...
    async _checkIfReleaseIsPublishedAsync() {
        const lastCheckFile = path.join(this.rushGlobalFolder.nodeSpecificPath, 'rush-' + Rush_1.Rush.version, 'last-check.flag');
    ...
    async _queryIfReleaseIsPublishedAsync(registryUrl) {
        ...
        const { WebClient } = await Promise.resolve().then(() => __importStar(require('../../utilities/WebClient')));
    ...
    _syncTempShrinkwrap(subspace, variant, shrinkwrapFile) {
        const committedShrinkwrapFileName = subspace.getCommittedShrinkwrapFilePath(variant);
        if (shrinkwrapFile) {
            Utilities_1.Utilities.syncFile(committedShrinkwrapFileName, subspace.getTempShrinkwrapFilename());
            Utilities_1.Utilities.syncFile(committedShrinkwrapFileName, subspace.getTempShrinkwrapPreinstallFilename());
        }
        else {
            // Otherwise delete the temporary file
            node_core_library_1.FileSystem.deleteFile(subspace.getTempShrinkwrapFilename());
            if (this.rushConfiguration.packageManager === 'pnpm') {
                ...
                node_core_library_1.FileSystem.deleteFile(path.join(subspace.getSubspaceTempFolderPath(), pnpmPackageManager.internalShrinkwrapRelativePath));
            }
        }
    }
    async validateNpmSetupAsync() {
        ...
        if (!this.options.bypassPolicy) {
            ...
            if (!valid) {
                ...
                console.error();
                // eslint-disable-next-line no-console
                console.error(terminal_1.Colorize.red('ERROR: NPM credentials are missing or expired'));
                // eslint-disable-next-line no-console
                console.error();
                // eslint-disable-next-line no-console
                console.error(terminal_1.Colorize.bold('==> Please run "rush setup" to update your NPM token. ' +
                    `(Or append "${RushConstants_1.RushConstants.bypassPolicyFlagLongName}" to proceed anyway.)`));
                throw new node_core_library_1.AlreadyReportedError();
            }
        }
