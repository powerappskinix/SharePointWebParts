...
const getHashesForGlobsAsync_1 = require("../buildCache/getHashesForGlobsAsync");
...
class CacheableOperationPlugin {
    ...
    apply(hooks) {
        ...
        hooks.beforeExecuteOperations.tapPromise(PLUGIN_NAME, async (recordByOperation, context) => {
            ...
            for (const operation of recordByOperation.keys()) {
                if (((_a = operation.settings) === null || _a === void 0 ? void 0 : _a.allowCobuildWithoutCache) &&
                    !(cobuildConfiguration === null || cobuildConfiguration === void 0 ? void 0 : cobuildConfiguration.cobuildWithoutCacheAllowed)) {
                    throw new Error(`Operation ${operation.name} is not allowed to run without the cobuild orchestration experiment enabled. You must enable the "allowCobuildWithoutCache" experiment in experiments.json.`);
                }
                if (((_b = operation.settings) === null || _b === void 0 ? void 0 : _b.allowCobuildWithoutCache) &&
                    !operation.settings.disableBuildCacheForOperation) {
                    throw new Error(`Operation ${operation.name} must have disableBuildCacheForOperation set to true when using the cobuild orchestration experiment. This is to prevent implicit cache dependencies for this operation.`);
                }
            }
            await node_core_library_1.Async.forEachAsync(recordByOperation.keys(), async (operation) => {
                    ...
                    if ((cobuildConfiguration === null || cobuildConfiguration === void 0 ? void 0 : cobuildConfiguration.cobuildFeatureEnabled) && cobuildConfiguration.cobuildContextId) {
                        ...
                        for (const operation of groupedOperations) {
                            ...
                            if (project && phase) {
                                ...
                                hash.update((_c = operation.name) !== null && _c !== void 0 ? _c : phase.name);
                                hash.update(RushConstants_1.RushConstants.hashDelimiter);
                            }
                        }
        ...
        hooks.afterExecuteOperation.tapPromise(PLUGIN_NAME, async (runnerContext) => {
            ...
            if (!project || !phase || !(runner === null || runner === void 0 ? void 0 : runner.cacheable) || !operationMetadataManager) {
                return;
            }
    ...
    async _tryGetProjectBuildCacheAsync({ buildCacheConfiguration, buildCacheContext, rushProject, phase, configHash, terminal, operationMetadataManager, operation }) {
        var _a;
        if (!buildCacheContext.projectBuildCache) {
            ...
            buildCacheContext.projectBuildCache = await ProjectBuildCache_1.ProjectBuildCache.tryGetProjectBuildCacheAsync({
                project: rushProject,
    ...
    async _tryGetLogOnlyProjectBuildCacheAsync({ buildCacheContext, rushProject, terminal, configHash, buildCacheConfiguration, cobuildConfiguration, phase, operationMetadataManager }) {
        ...
        const projectBuildCache = await ProjectBuildCache_1.ProjectBuildCache.tryGetProjectBuildCacheAsync({
            project: rushProject,
            projectOutputFolderNames,
            additionalProjectOutputFilePaths,
            additionalContext,
...
async function updateAdditionalContextAsync({ operationSettings, additionalContext, projectChangeAnalyzer, terminal, rushProject }) {
    ...
    if (operationSettings.dependsOnAdditionalFiles) {
        ...
        const additionalFiles = await (0, getHashesForGlobsAsync_1.getHashesForGlobsAsync)(operationSettings.dependsOnAdditionalFiles, rushProject.projectFolder, repoState);
