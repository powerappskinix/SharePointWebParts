...
const TIMELINE_CHART_SYMBOLS = {
    [OperationStatus_1.OperationStatus.Waiting]: '?',
    [OperationStatus_1.OperationStatus.Ready]: '?',
    [OperationStatus_1.OperationStatus.Queued]: '?',
...
const TIMELINE_CHART_COLORIZER = {
    [OperationStatus_1.OperationStatus.Waiting]: terminal_1.Colorize.yellow,
    [OperationStatus_1.OperationStatus.Ready]: terminal_1.Colorize.yellow,
    [OperationStatus_1.OperationStatus.Queued]: terminal_1.Colorize.yellow,
    [OperationStatus_1.OperationStatus.Executing]: terminal_1.Colorize.yellow,
    [OperationStatus_1.OperationStatus.Success]: terminal_1.Colorize.green,
    [OperationStatus_1.OperationStatus.SuccessWithWarning]: terminal_1.Colorize.yellow,
    [OperationStatus_1.OperationStatus.Failure]: terminal_1.Colorize.red,
    [OperationStatus_1.OperationStatus.Blocked]: terminal_1.Colorize.red,
    [OperationStatus_1.OperationStatus.Skipped]: terminal_1.Colorize.green,
    [OperationStatus_1.OperationStatus.FromCache]: terminal_1.Colorize.green,
    [OperationStatus_1.OperationStatus.NoOp]: terminal_1.Colorize.gray
};
/**
...
function _printTimeline({ terminal, result, cobuildConfiguration }) {
    ...
    //
    const maxWidth = terminal_1.PrintUtilities.getConsoleWidth() || TIMELINE_WIDTH;
    ...
    for (const record of data) {
        ...
        const chart = terminal_1.Colorize.gray('-'.repeat(startIdx)) +
            TIMELINE_CHART_COLORIZER[status](getChartSymbol(record).repeat(length)) +
            terminal_1.Colorize.gray('-'.repeat(chartWidth - endIdx));
        terminal.writeLine(`${terminal_1.Colorize.cyan(name.padStart(longestNameLength))} ${chart} ${terminal_1.Colorize.white(durationString.padStart(longestDurationLength) + 's')}`);
    }
    terminal.writeLine('='.repeat(maxWidth));
    //
    ...
    if (durationByPhase.size > 0) {
        ...
        for (const [phase, duration] of durationByPhase.entries()) {
            terminal.writeLine(`  ${terminal_1.Colorize.cyan(phase.name.padStart(maxPhaseName))} ${duration.toFixed(1)}s`);
        }
    }
    terminal.writeLine('');
}
