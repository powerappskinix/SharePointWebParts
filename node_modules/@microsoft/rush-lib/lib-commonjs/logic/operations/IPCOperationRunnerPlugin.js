...
exports.IPCOperationRunnerPlugin = void 0;
const IPCOperationRunner_1 = require("./IPCOperationRunner");
...
class IPCOperationRunnerPlugin {
    apply(hooks) {
        ...
        hooks.createOperations.tapPromise({
            name: PLUGIN_NAME,
            before: ShellOperationRunnerPlugin_1.PLUGIN_NAME
            ...
            for (const operation of operations) {
                ...
                const rawScript = (_a = project.packageJson.scripts) === null || _a === void 0 ? void 0 : _a[`${phase.name}:ipc`];
                if (!rawScript) {
                    continue;
                }
                const commandToRun = (0, ShellOperationRunnerPlugin_1.formatCommand)(rawScript, getCustomParameterValuesForPhase(phase));
                const operationName = (0, ShellOperationRunnerPlugin_1.getDisplayName)(phase, project);
                let maybeIpcOperationRunner = runnerCache.get(operationName);
                if (!maybeIpcOperationRunner) {
                    const ipcOperationRunner = (maybeIpcOperationRunner = new IPCOperationRunner_1.IPCOperationRunner({
                        phase,
                        project,
                        name: operationName,
                        ...
                        requestRun: (requestor) => {
                            ...
                            if (status === OperationStatus_1.OperationStatus.Waiting ||
                                status === OperationStatus_1.OperationStatus.Ready ||
                                status === OperationStatus_1.OperationStatus.Queued) {
        ...
        hooks.beforeExecuteOperations.tap(PLUGIN_NAME, (records, context) => {
            ...
            for (const [{ runner }, result] of records) {
                if (runner instanceof IPCOperationRunner_1.IPCOperationRunner) {
...
exports.IPCOperationRunnerPlugin = IPCOperationRunnerPlugin;
