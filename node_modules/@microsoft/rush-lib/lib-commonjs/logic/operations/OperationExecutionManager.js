...
class OperationExecutionManager {
    constructor(operations, options) {
        this._streamCollator_onWriterActive = (writer) => {
            if (writer) {
                ...
                const leftPart = terminal_1.Colorize.gray('==[') + ' ' + terminal_1.Colorize.cyan(writer.taskName) + ' ';
                const leftPartLength = 4 + writer.taskName.length + 1;
                // rightPart: " 1 of 1000 ]=="
                const completedOfTotal = `${this._completedOperations} of ${this._totalOperations}`;
                const rightPart = ' ' + terminal_1.Colorize.white(completedOfTotal) + ' ' + terminal_1.Colorize.gray(']==');
                ...
                const middlePart = terminal_1.Colorize.gray(']' + '='.repeat(middlePartLengthMinusTwoBrackets) + '[');
        ...
        const { quietMode, debugMode, parallelism, changedProjectsOnly, beforeExecuteOperationAsync: beforeExecuteOperation, afterExecuteOperationAsync: afterExecuteOperation, onOperationStatusChangedAsync: onOperationStatusChanged, beforeExecuteOperationsAsync: beforeExecuteOperations } = options;
        ...
        this._onOperationStatusChanged = (record) => {
            if (record.status === OperationStatus_1.OperationStatus.Ready) {
        ...
        this._colorsNewlinesTransform = new terminal_1.TextRewriterTransform({
            ...
            removeColors: !terminal_1.ConsoleTerminalProvider.supportsColor
        });
    ...
    async executeAsync() {
        ...
        const onOperationCompleteAsync = async (record) => {
            ...
            try {
                ...
                record.error = e;
                record.status = OperationStatus_1.OperationStatus.Failure;
            }
        ...
        await node_core_library_1.Async.forEachAsync(this._executionQueue, async (operation) => {
    ...
    _reportOperationErrorIfAny(record) {
        ...
        if (record.error) {
            if (!(record.error instanceof node_core_library_1.AlreadyReportedError)) {
    ...
    _onOperationComplete(record) {
        ...
        switch (status) {
            ...
            case OperationStatus_1.OperationStatus.Failure: {
                ...
                terminal.writeStderrLine(terminal_1.Colorize.red(`"${name}" failed to build.`));
                const blockedQueue = new Set(record.consumers);
                for (const blockedRecord of blockedQueue) {
                    if (blockedRecord.status === OperationStatus_1.OperationStatus.Waiting) {
                    ...
                    else if (blockedRecord.status !== OperationStatus_1.OperationStatus.Blocked) {
                        // It shouldn't be possible for operations to be in any state other than Waiting or Blocked
                        throw new node_core_library_1.InternalError(`Blocked operation ${blockedRecord.name} is in an unexpected state: ${blockedRecord.status}`);
                    }
                }
            ...
            case OperationStatus_1.OperationStatus.FromCache: {
                if (!silent) {
                    record.collatedWriter.terminal.writeStdoutLine(terminal_1.Colorize.green(`"${name}" was restored from the build cache.`));
                }
                break;
            }
            /**
            ...
            case OperationStatus_1.OperationStatus.Skipped: {
                if (!silent) {
                    record.collatedWriter.terminal.writeStdoutLine(terminal_1.Colorize.green(`"${name}" was skipped.`));
                }
                break;
            }
            /**
            ...
            case OperationStatus_1.OperationStatus.NoOp: {
                if (!silent) {
                    record.collatedWriter.terminal.writeStdoutLine(terminal_1.Colorize.gray(`"${name}" did not define any work.`));
                }
                break;
            }
            case OperationStatus_1.OperationStatus.Success: {
                if (!silent) {
                    record.collatedWriter.terminal.writeStdoutLine(terminal_1.Colorize.green(`"${name}" completed successfully in ${record.stopwatch.toString()}.`));
                }
                break;
            }
            case OperationStatus_1.OperationStatus.SuccessWithWarning: {
                if (!silent) {
                    record.collatedWriter.terminal.writeStderrLine(terminal_1.Colorize.yellow(`"${name}" completed with warnings in ${record.stopwatch.toString()}.`));
                }
