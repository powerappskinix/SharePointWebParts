...
function _printOperationStatus(terminal, result) {
    ...
    writeCondensedSummary(terminal, OperationStatus_1.OperationStatus.Skipped, operationsByStatus, terminal_1.Colorize.green, 'These operations were already up to date:');
    writeCondensedSummary(terminal, OperationStatus_1.OperationStatus.NoOp, operationsByStatus, terminal_1.Colorize.gray, 'These operations did not define any work:');
    writeCondensedSummary(terminal, OperationStatus_1.OperationStatus.FromCache, operationsByStatus, terminal_1.Colorize.green, 'These operations were restored from the build cache:');
    writeCondensedSummary(terminal, OperationStatus_1.OperationStatus.Success, operationsByStatus, terminal_1.Colorize.green, 'These operations completed successfully:');
    writeDetailedSummary(terminal, OperationStatus_1.OperationStatus.SuccessWithWarning, operationsByStatus, terminal_1.Colorize.yellow, 'WARNING');
    writeCondensedSummary(terminal, OperationStatus_1.OperationStatus.Blocked, operationsByStatus, terminal_1.Colorize.white, 'These operations were blocked by dependencies that failed:');
    writeDetailedSummary(terminal, OperationStatus_1.OperationStatus.Failure, operationsByStatus, terminal_1.Colorize.red);
    terminal.writeLine('');
...
function writeDetailedSummary(terminal, status, operationsByStatus, headingColor, shortStatusName) {
    ...
    for (const [operation, operationResult] of operations) {
        ...
        terminal.writeLine(`${terminal_1.Colorize.gray('--[')} ${headingColor(subheadingText)} ${terminal_1.Colorize.gray(`]${'-'.repeat(middlePartLengthMinusTwoBrackets)}[`)} ${terminal_1.Colorize.white(time)} ${terminal_1.Colorize.gray(']--')}\n`);
...
function writeSummaryHeader(terminal, status, operations, headingColor) {
    ...
    // rightPart: "]======================"
    terminal.writeLine(`${terminal_1.Colorize.gray('==[')} ${headingColor(headingText)} ${terminal_1.Colorize.gray(`]${'='.repeat(rightPartLengthMinusBracket)}`)}\n`);
}
