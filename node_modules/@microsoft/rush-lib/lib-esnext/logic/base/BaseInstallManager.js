...
export class BaseInstallManager {
    ...
    async doInstallAsync() {
        ...
        if (resolutionOnly ||
            ...
            if (this.options.allowShrinkwrapUpdates && !shrinkwrapIsUpToDate) {
                ...
                const shrinkwrapFile = ShrinkwrapFileFactory.getShrinkwrapFile(this.rushConfiguration.packageManager, this.rushConfiguration.packageManagerOptions, committedShrinkwrapFileName);
    ...
    async prepareAsync(subspace, variant, projectImpactGraphGenerator) {
        ...
        if (!this.options.fullUpgrade) {
            const committedShrinkwrapFileName = subspace.getCommittedShrinkwrapFilePath(variant);
            try {
                shrinkwrapFile = ShrinkwrapFileFactory.getShrinkwrapFile(this.rushConfiguration.packageManager, this.rushConfiguration.packageManagerOptions, committedShrinkwrapFileName);
            }
            catch (ex) {
                terminal.writeLine();
...
`;
    ...
    pushConfigurationArgs(args, options, subspace) {
        ...
        if (this.rushConfiguration.packageManager === 'npm') {
            ...
            */
            const isAutoInstallPeersInNpmrc = isVariableSetInNpmrcFile(subspace.getSubspaceConfigFolderPath(), 'auto-install-peers');
            ...
            */
            const isResolutionModeInNpmrc = isVariableSetInNpmrcFile(subspace.getSubspaceConfigFolderPath(), 'resolution-mode');
