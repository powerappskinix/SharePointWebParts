...
var ImplicitFlowTokenProvider = /** @class */ (function () {
    ...
    ImplicitFlowTokenProvider.prototype.getToken = function (resourceEndpoint, optionsOrUseCachedToken) {
        var _this = this;
        ...
        var extraData = this._tokenRequestManager._generateTelemetryExtraData(resourceEndpoint, this._configuration.spRequestGuid);
        var cachedToken = this._tokenStorage.getToken(resourceEndpoint);
        if (useCachedToken && cachedToken) {
            extraData.alias = 'true';
            acquireAccessTokenQosMonitor.writeSuccess(extraData);
            return Promise.resolve(cachedToken);
        }
        return this._tokenRequestManager
            .getToken(resourceEndpoint, true)
            .then(function (response) {
            if (TokenRequestManager_1.TokenRequestManager.isSuccessfulResponse(response)) {
                _this._tokenStorage.saveToken(response, resourceEndpoint);
                acquireAccessTokenQosMonitor.writeSuccess(extraData);
    ...
    ImplicitFlowTokenProvider.prototype._ensureState = function () {
        return Promise.resolve();
    };
    ImplicitFlowTokenProvider.prototype._ensureTelemetry = function () {
        return Promise.resolve();
    };
