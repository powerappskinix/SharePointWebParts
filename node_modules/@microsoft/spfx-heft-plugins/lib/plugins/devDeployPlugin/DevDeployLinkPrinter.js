...
function _printHelperMessage(terminal, queryParameters) {
    const message = `For an improved debugging experience, please set the ` +
        ...
        ` testing.`;
    terminal.writeLine(terminal_1.Colorize.yellow(message));
    terminal.writeLine(..._getAdvice());
    terminal.writeLine(terminal_1.Colorize.yellow('Debug query parameters:'), ` "${_concatenateQueryParams(queryParameters)}"`);
}
function _getAdvice() {
    ...
    const osAdvice = {
        win32: [
            ...
            terminal_1.Colorize.yellow(`set ${TEST_URL_ENVIRONMENT_VARIABLE}=${sampleTestPageUrl}`)
        ],
        darwin: [
            `For example, in MacOS:${os.EOL}`,
            terminal_1.Colorize.yellow(`export ${TEST_URL_ENVIRONMENT_VARIABLE}=${sampleTestPageUrl}`)
        ],
        default: [
...
function _printDeployString(terminal, testingPageUrl, queryParams) {
    ...
    terminal.writeLine(terminal_1.Colorize.yellow('Test URL:'), ` ${testUrl}`);
}
/**
