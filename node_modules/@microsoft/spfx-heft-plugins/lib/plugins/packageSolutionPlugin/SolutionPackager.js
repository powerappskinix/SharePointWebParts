...
const package_solution_schema_json_1 = __importDefault(require("./package-solution.schema.json"));
...
class SolutionPackager {
    ...
    async packageSolutionAsync(production, prettyPrintXml = false, noSpppkg = false) {
        ...
        const manifestFilenames = await (0, fast_glob_1.default)('*.manifest.json', {
        ...
        if (config.solution.includeClientSideAssets) {
            const assetsPaths = await (0, fast_glob_1.default)('**/*.*', {
                ignore: ignoredAssets,
                cwd: assetsFolderPath
            });
            assets = assetsPaths.map((filename) => {
                this._terminal.writeLine(terminal_1.Colorize.cyan(`Found client-side build resource: ${filename}`));
                return {
        ...
        const teamsIconPaths = await (0, fast_glob_1.default)('**/*.*', {
            ignore: ignoredAssets,
            cwd: config.paths.teamsDir
        });
        const teamsIcons = teamsIconPaths.map((filename) => {
            this._terminal.writeLine(terminal_1.Colorize.cyan(`Found teams icons: ${filename}`));
            return {
        ...
        if (metadata && metadata.screenshotPaths) {
            ...
            for (let screenshotPathIdx = 0, screenshotPathsLen = metadata.screenshotPaths.length; screenshotPathIdx < screenshotPathsLen; screenshotPathIdx++) {
                ...
                if (!UrlUtilities_1.UrlUtilities.isUrl(screenshotPath)) {
                    ...
                    if (screenshotExists) {
                        ...
                        this._terminal.writeLine(terminal_1.Colorize.cyan(`Found screenshot: ${screenshotFilename}`));
                        screenshots.push({
            ...
            if (config.solution.iconPath) {
                ...
                if (iconExists) {
                    ...
                    this._terminal.writeLine(terminal_1.Colorize.cyan(`Found icon: ${iconName}`));
...
SolutionPackager._configFile = new heft_config_file_1.ConfigurationFile({
    ...
    jsonSchemaObject: package_solution_schema_json_1.default
});
