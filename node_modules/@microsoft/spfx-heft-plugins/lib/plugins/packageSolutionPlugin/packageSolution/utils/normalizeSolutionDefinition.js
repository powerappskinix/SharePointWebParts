...
function createFeatureFromComponent(component) {
    return {
        ...
        description: `A feature which activates the Client-Side ${component.manifest.componentType} named ` +
            `${component.name}`,
        id: component.id || uuid.v4(), // use the component id if it exists (for a normal component),
...
function normalizeSolutionDefinition(terminal, options, manifests) {
    ...
    if (options.solution && options.solution.features) {
        ...
        for (const feature of options.solution.features) {
            ...
            feature.assets.elementFiles = resolveFeatureFiles(
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            feature.assets.elementFiles, options.paths.sharepointAssetDir);
            feature.assets.elementManifests = resolveFeatureFiles(
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            feature.assets.elementManifests, options.paths.sharepointAssetDir);
            feature.assets.upgradeActions = resolveFeatureFiles(
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
